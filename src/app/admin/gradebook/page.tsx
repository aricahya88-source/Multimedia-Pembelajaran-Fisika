...
export default function Gradebook(){
  ...
  const pending=useMemo(()=>uniqueEvidence.filter(r=>!r.has_comment),[uniqueEvidence]);
  const graded=useMemo(()=>uniqueEvidence.filter(r=>!!r.grade).length,[uniqueEvidence]);

  const exportPending=async()=>{
    if(!current)return;if(current.type==='quiz'){setMsg('Kuis dinilai otomatis dan tidak masuk export penilaian manual.');return;}if(!pending.length){setMsg('Semua hasil pada aktivitas ini sudah memiliki komentar dosen.');return;}
    ...
    const info=[
      ...
      ['1. File ini berisi hasil yang belum memiliki komentar dosen.'],
      ['2. Isi kolom Komentar dengan feedback dosen. Kolom ini wajib diisi.'],
      ['3. Kolom Nilai bersifat opsional. Kosongkan bila hanya ingin mengimpor komentar tanpa mengubah/menambahkan nilai.'],
    ...
    const safe=current.title.replace(/[^a-z0-9\-_]+/gi,'_').slice(0,55);XLSX.writeFile(wb,`Butuh_Komentar_${safe}_${new Date().toISOString().slice(0,10)}.xlsx`);
    setMsg(`${pending.length} hasil yang membutuhkan komentar berhasil diekspor.`);
  };
  ...
  const importErrors=useMemo(()=>importRows.map((r,i)=>{const errs:string[]=[];const n=r.score===''?null:Number(r.score);if(!r.activity_id||r.activity_id!==activity)errs.push('aktivitas tidak sesuai');if(!r.user_id)errs.push('user_id kosong');if(n!==null&&(Number.isNaN(n)||n<0||n>100))errs.push('nilai harus kosong atau 0–100');if(!r.comment)errs.push('komentar kosong');return errs.length?`Baris ${i+2}: ${errs.join(', ')}`:''}).filter(Boolean),[importRows,activity]);
  ...
  const runImport=async()=>{
    ...
    try{for(const chunk of chunks){const result=await api<ImportReport>('adminImportGrades',{rows:chunk.map(r=>({activity_id:r.activity_id,user_id:r.user_id,submission_id:r.submission_id,group_id:r.group_id,score:r.score===''?null:Number(r.score),comment:r.comment}))});total.processed+=result.processed;total.grade_records+=result.grade_records;total.comments+=result.comments;total.group_rows+=result.group_rows;total.errors.push(...result.errors)}setImportReport(total);setMsg(`Import selesai: ${total.processed} baris diproses, ${total.grade_records} nilai tersimpan, ${total.comments} komentar dicatat.`);setImportRows([]);setImportName('');await loadRows()}catch(e){setImportReport(total);setMsg(e instanceof Error?e.message:String(e))}finally{setImportBusy(false)}
  };

  return <AuthGate adminOnly><AppShell title="Gradebook"><div className="stack">
    <GlassCard><div className="row between wrap gap"><label className="field grow"><span>Pilih aktivitas</span><select value={activity} onChange={e=>setActivity(e.target.value)}>{activities.map(a=><option key={a.activity_id} value={a.activity_id}>{a.week_id} • {a.title} ({a.type})</option>)}</select></label><div className="row wrap gap"><button className="button soft" disabled={!current||current.type==='quiz'||pending.length===0} onClick={exportPending}><Download/>Export {pending.length} untuk Komentar</button><label className="button primary"><Upload/>Import Komentar / Nilai<input className="hidden" type="file" accept=".xlsx,.xls" onChange={e=>readImport(e.target.files?.[0]||null)}/></label></div></div><p className="muted">Export mengambil hasil yang <strong>belum memiliki komentar dosen</strong>. Saat import, kolom Komentar wajib diisi sedangkan Nilai bersifat opsional (0–100). Dengan demikian dosen dapat mengimpor komentar saja tanpa memberi/mengubah nilai. Kuis tetap dinilai otomatis.</p></GlassCard>

    <div className="gradebook-stat-grid"><GlassCard><span className="eyebrow">HASIL MASUK</span><h2>{uniqueEvidence.length}</h2><small>submission/post yang siap diperiksa</small></GlassCard><GlassCard><span className="eyebrow">SUDAH DINILAI</span><h2>{graded}</h2><small>nilai tersimpan pada skala 100</small></GlassCard><GlassCard><span className="eyebrow">BUTUH KOMENTAR</span><h2>{pending.length}</h2><small>akan masuk file export Excel</small></GlassCard></div>

    {importRows.length>0&&<GlassCard><div className="row between wrap gap"><div><span className="eyebrow">PREVIEW IMPORT</span><h3>{importName}</h3></div><div className="row wrap gap"><span className="badge success"><CheckCircle2/> {validImport} siap</span>{importErrors.length>0&&<span className="badge danger"><AlertTriangle/> {importErrors.length} masalah</span>}</div></div><div className="scroll-list">{importRows.slice(0,8).map((r,i)=><div className="select-row" key={`${r.user_id}-${i}`}><strong>{r.name||r.nim||r.user_id}</strong><small>Nilai {r.score===''?'opsional':r.score} • {r.comment?`${r.comment.slice(0,100)}${r.comment.length>100?'…':''}`:'komentar kosong'}</small></div>)}</div>{importErrors.length>0&&<div className="error-box selectable">{importErrors.slice(0,10).map((x,i)=><div key={i}>{x}</div>)}</div>}<div className="right-actions"><button className="button primary" disabled={importBusy||importErrors.length>0||!importRows.length} onClick={runImport}><FileSpreadsheet/>{importBusy?'Mengimpor...':`Import ${validImport} Baris`}</button></div></GlassCard>}
    {importReport&&<div className="notice"><strong>Import komentar/nilai selesai.</strong> {importReport.processed} baris diproses, {importReport.grade_records} record nilai tersimpan, {importReport.comments} komentar ditambahkan.{importReport.group_rows>0&&<> <UsersRound size={15}/> {importReport.group_rows} baris kelompok diterapkan ke seluruh anggota.</>}{importReport.errors.map((x,i)=><div key={i}>{x}</div>)}</div>}
