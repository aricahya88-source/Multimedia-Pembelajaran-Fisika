...
export default function AdminActivities(){
  ...
  return <AuthGate adminOnly><AppShell title="Kelola Tugas">
    <div className="admin-split"><GlassCard className="admin-list"><div className="row between"><div><span className="eyebrow">AKTIVITAS</span><h3>Proyek, Checkpoint, & Partisipasi</h3></div><button className="icon-button" onClick={()=>setA({...empty})}><Plus/></button></div><div className="scroll-list">{rows.map(x=><button key={x.activity_id} className={a.activity_id===x.activity_id?'select-row active':'select-row'} onClick={()=>setA(x)}><strong>{x.title}</strong><small>{x.week_id} • {x.type}</small></button>)}</div></GlassCard>
    <GlassCard>
      <div className="form-grid two">
        <label className="field"><span>Pertemuan</span><select value={a.week_id} onChange={e=>setA({...a,week_id:e.target.value})}>{Array.from({length:16},(_,i)=><option key={i} value={`W${String(i+1).padStart(2,'0')}`}>Pertemuan {i+1}</option>)}</select></label>
      ...
      {a.type==='project'&&<label className="field"><span>Project code</span><input value={a.project_code||''} onChange={e=>setA({...a,project_code:e.target.value.toUpperCase()})} placeholder="DESIGN_PROJECT / IMPLEMENTATION_PROJECT"/></label>}
      </div>
